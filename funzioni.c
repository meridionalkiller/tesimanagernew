...
void ricerca(FILE *file) //richiesta del metodo di ricerca
{   
    char scelta_str[3];
    unsigned int scelta;
    do
...
{
    for (int i = 0; str[i]; i++) {
        str[i] = tolower(str[i]); //rende tutte le lettere delle stringhe minuscole
    }
}
...
void aggiornatesi(FILE *file) //funzione per l'aggiornamento di una specifica tesi
{
    ...
    int trovato = 0;
    long posizione; 
    do //controllo dell codice inserito
    {
    ...
    rewind(file);
     trovato= ricercaCodice(file, codice, &stud);
        if (trovato) 
        {
            ...
            do 
            {
                do //richiesta della modifica che l'utente vuole effettuare
                {
                ...
                if (mod == 1) //modifica del titolo tesi
                {
                    printf("\nTitolo tesi: %s\n", stud.titolo_tesi); //scrittura del valore prima della modifica
                    printf("Inserisci il titolo della tesi modificato: ");
                    getchar(); 
                    fgets(stud.titolo_tesi, sizeof(stud.titolo_tesi), stdin); //lettura nuovo valore
                }
                else if (mod == 2) //modifica della materia
                {
                    ...
                    do
                    {
                        ...
                        char scelta_str[3];
                        scanf("%s", scelta_str); //lettura nuovo valore
                        scelta = atoi(scelta_str);
                        if (scelta < 1 || scelta > 6)
                        {
                            printf("\nScelta non valida.\n");
                ...
                {   
                    ...
                    getchar();
                    scanf("%s", stud.parole_chiave); //lettura nuovo valore
                    printf("Ora la seconda: ");
                    getchar();
                    scanf("%s", stud.parole_chiave1); //lettura nuovo valore
                    printf("Ora la terza: ");
                    getchar();
                    scanf("%s", stud.parole_chiave2); //lettura nuovo valore
                }
                else if (mod == 4) //modifica dell'anno accademico
                {
                    ...
                    do
                    {
                        ...
                        char anno_accademico_str[10];
                        scanf("%s", anno_accademico_str);//lettura nuovo valore
                        stud.anno_accademico = atoi(anno_accademico_str);
                        if (stud.anno_accademico < 2005 || stud.anno_accademico > 2030)
                        {
                            printf("Anno accademico non valido. Riprova.\n");
                ...
                else if (mod == 6) //modifica del relatore
                {
                    ...
                    getchar(); 
                    fgets(stud.relatore, sizeof(stud.relatore), stdin); //lettura nuovo valore
                }
                ...
                fflush(file);
            do
                { //richiesta di modifica di un altro elemento
...
void cancellatesi(FILE* file) //funzione per cancellare una tesi
{
    ...
    {
        stud.cancellato = 1; 
        posizione = (codice - 1) * sizeof(studente);
        fseek(file, posizione, SEEK_SET);
        fwrite(&stud, sizeof(studente), 1, file); //sovrascrive il valore di stud.cancellato impostandolo a 1
        fflush(file);
...
void stampa(FILE *file) //funzione che stampa il file ordinato secondo l'ordinamento del bubble sort
{
    ...
    for (int i = 0; i < nTesi; i++) {
        ...
        if (stud.cancellato == 0)
        {
            ...
            printf("---------------------------------\n");
            printf("Studente: %s %s", stud.nome,stud.cognome);
            printf("Numero tesi: %u \n", stud.n_tesi);
            printf("Titolo tesi: %s", stud.titolo_tesi);
            printf("Materia: ");
            switch (stud.mat)
...
{   
    ...
    do //controllo dell codice inserito
    {
        ...
        {   
            posizione = (codice - 1) * sizeof(studente); // Salva la posizione del record da modificare
            temposcaricamento(); //utilizzato per la simulazione del download
            printf("Nome: %s\n", stud.nome);
            printf("Cognome: %s", stud.cognome);
            printf("Numero tesi: %u \n", stud.n_tesi);
            printf("Titolo tesi: %s", stud.titolo_tesi);
